...
from maths_function import quintic_smooth, lerp
...
class PerlinNoise:
    ...
    def get_value(p):
        if isinstance(p, Vector2):
            return PerlinNoise.get_value(p.to_vector3(0.0))
        elif isinstance(p, Vector3):
            x, y, z = p.x, p.y, p.z
            unit_x = int(math.floor(x)) & 255
            unit_y = int(math.floor(y)) & 255
            unit_z = int(math.floor(z)) & 255
            x -= math.floor(x)
            y -= math.floor(y)
            z -= math.floor(z)
            ...
            l1 = lerp(PerlinNoise.gradient(Perm[aa], x, y, z), PerlinNoise.gradient(Perm[ba], x - 1, y, z), u)
            l2 = lerp(PerlinNoise.gradient(Perm[ab], x, y - 1, z), PerlinNoise.gradient(Perm[bb], x - 1, y - 1, z), u)
            l3 = lerp(PerlinNoise.gradient(Perm[aa + 1], x, y, z - 1), PerlinNoise.gradient(Perm[ba + 1], x - 1, y, z - 1), u)
            l4 = lerp(PerlinNoise.gradient(Perm[ab + 1], x, y - 1, z - 1), PerlinNoise.gradient(Perm[bb + 1], x - 1, y - 1, z - 1), u)
            l5 = lerp(l1, l2, v)
            l6 = lerp(l3, l4, v)
            return lerp(l5, l6, w)
    ...
    @staticmethod
    def get_value2(p):
        return PerlinNoise.get_value(Vector3(p.x, p.y, 0.0))

    @staticmethod
    def get_value3(p):
        return PerlinNoise.get_value(p.x, p.y, p.z)